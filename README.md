...
- `src/pages/*.astro` — de zeven pagina's. `/vacature` is noindex en staat buiten de sitemap.
- `public/assets/` — logo-SVG's (zonder C2PA-metadata). `public/images/` — WebP-afbeeldingen. `public/og.png` — deelafbeelding.

## Formulieren

`/gesprek` en `/indicatie` posten JSON naar een formulierdienst (Formspree, Web3Forms of Basin). Zet in Vercel de omgevingsvariabelen uit `.env.example`:
...
Zonder endpoint toont het formulier de foutmelding met `tim@finable.nl` als terugvaloptie; een bevestiging verschijnt alleen na een succesvolle response.
